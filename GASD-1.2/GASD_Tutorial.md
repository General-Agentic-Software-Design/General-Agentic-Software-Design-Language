...
>
> **GASD Rule #2**: All design elements **MUST** align with the [Official Self-Definition](gasd-1.2.0.gasd).
...
TYPE HttpError:
    status: 404         // Literal Type: Must be exactly 404
    message: String
```

> [!NOTE]
> GASD supports **Literal Types** (like `404` or `"SUCCESS"`). Instead of specifying a broad shape (`Integer`), you can enforce that a variable must match an exact literal value. This bridges the gap to modern discriminated unions.
...
### 3.2 ACHIEVE with POSTCONDITION *(GASD 1.2 addition)*

In GASD 1.2, you must formally define the expected outcome of an `ACHIEVE` step using a `POSTCONDITION` expression. This makes the high-level goal verifiable.

```gasd
FLOW create_account(email: String):
    1. ACHIEVE "Register external billing customer"
        POSTCONDITION: result.customerId IS UUID
        ON_ERROR: THROW BillingException("Registration failed")
    2. RETURN result.customerId
```

You can chain multiple postconditions and set timeouts:

```gasd
    1. ACHIEVE "Sync database records"
        POSTCONDITION:
...
### 3.3 Explicit Step Dependencies: `DEPENDS_ON` *(GASD 1.2 addition)*

In GASD 1.1, step execution order and dependencies were inferred. GASD 1.2 introduces the `DEPENDS_ON` clause to structure unambiguous, verifiable execution graphs. This is particularly important for concurrent or independent tasks.
...
### 4.3 Defining CONTRACTs at IMPORT Boundaries *(GASD 1.2 addition)*

When building large systems with GASD 1.2, you cannot just indiscriminately `IMPORT` another component without knowing its specific behavior. A `CONTRACT` explicitly models what you expect from that imported component.

```gasd
NAMESPACE: "orders"
IMPORT "../services/payment.gasd" AS Payment

CONTRACT Payment.chargeCard:
    INPUT: amount: Decimal, token: String
    OUTPUT: TransactionReceipt
    BEHAVIORS:
        CASE success:
            POSTCONDITION: result.status == "CLEARED"
        CASE declined:
...
The agent uses this contract to generate all necessary error handling, retries, and fallback logic in your `orders/` code, guaranteeing safety at the architectural boundary.
...
### 8.5 Adding GASD 1.2 Constructs: Verification and Rigor

Now let's upgrade our notification service with GASD 1.2 features to guarantee verification and strict behavioral contracts.
...
// 1. Declare explicit assumptions about the environment
ASSUMPTION "Push Notification Provider has 99.9% uptime":
    AFFECTS: [NotificationService.send_alert]
    CONSEQUENCE: "Alerts may be dropped if provider is down"

// 2. Add an INVARIANT with explicit scope
INVARIANT GLOBAL: "High priority alerts must never be silently dropped"

// 3. Define a CONTRACT for our external dependency
IMPORT "providers/email.gasd" AS EmailProvider

CONTRACT EmailProvider.send:
...
// 4. Update the FLOW with new 1.2 primitives
FLOW send_alert(request) -> Result<Void>:
    1. VALIDATE request AS TYPE.AlertRequest
    
    // Use POSTCONDITION and DEPENDS_ON to formally structure the achievement
    2. ACHIEVE "Format message body"
        POSTCONDITION: result.body IS String
        
    MATCH request.priority:
        HIGH -> 
            3. ACHIEVE "Send Push" DEPENDS_ON STEP 2
                POSTCONDITION: result.delivered == true
                TIMEOUT: 1000ms
            
            4. ACHIEVE "Send Email" DEPENDS_ON STEP 2
                POSTCONDITION: result.delivered == true
        
        MEDIUM ->
            5. ACHIEVE "Send Push" DEPENDS_ON STEP 2
                POSTCONDITION: result.delivered == true
        
        // Use the AS obligation identifier for traceability
        LOW -> 
            6. ACHIEVE "Log to Buffer" AS LOGGING_OBLIGATION
                POSTCONDITION: result == void
                
    RETURN Result.success
```

By adding these constructs:

- **`VERSION 1.2`** forces all tools to strictly check these rules.
- **`ASSUMPTION`** makes our external reliance explicit for ops teams.
- **`CONTRACT`** formally models the errors our code must handle from `EmailProvider`.
- **`POSTCONDITION`** makes every `ACHIEVE` decidable to an analyzer.
- **`DEPENDS_ON`** guarantees the execution graph structure.
- **`AS` identifiers** create stable tracking for quality metrics.
...
1. **User Registration**: [user_registration.gasd](../examples/multi_doc_dep/user_registration.gasd) - CRUD service with validation.
2. **E-commerce Checkout**: [ecommerce_checkout.gasd](../examples/ecommerce_checkout.gasd) - Dependency injection and persistence.
3. **User Management System**: [user_management.gasd](../examples/multi_doc_dep/user_management.gasd) - Full microservice with global architectural rules.

### 7.3 Advanced Distributed Systems

1. **Distributed SAGA Transaction**: [saga_transaction.gasd](../examples/saga_transaction.gasd) - Handling distributed failure with compensation.
2. **Raft Consensus (Lite)**: [raft_consensus.gasd](../examples/raft_consensus.gasd) - Strict state machine implementation.
3. **Autonomous Drone Navigation**: [drone_navigation.gasd](../examples/drone_navigation.gasd) - Human-in-the-Loop safety systems.
...
Always refer to the [Official Self-Definition (gasd-1.2.0.gasd)](gasd-1.2.0.gasd) for the most precise mapping of language constructs. AI agents use this file as their normative reference for what is possible within the GASD 1.2.0 standard.

---

## 12. Formal Verification Support (GASD 1.2)

One of the largest additions in GASD 1.2 is structural support for formal semantics and decidable correctness. To use GASD as a rigorous architectural contract, you can enforce the following guarantees:

### 12.1 Explicit Assumptions

Environmental dependencies (like databases or third-party uptime) must be declared openly using `ASSUMPTION`.
...
### 12.2 Scoped Invariants

`INVARIANT` rules now require an explicit scope (`LOCAL` or `GLOBAL`) to definitively dictate how thoroughly they must be enforced.

```gasd
// Component-scoped: verified within this component's boundary
INVARIANT LOCAL: "Account balance must never be negative"

// System-scoped: verified across all entries, DBs, and admin paths
INVARIANT GLOBAL: "User email must be unique in the system at all times"
```

### 12.3 Obligation Identifiers (`AS`)

To prevent "Agent Drift" during multi-step verification, GASD 1.2 allows assigning stable identifiers to annotations or actions using the `AS` keyword.

```gasd
TYPE RequestData:
    password: String
        @min_length(8) AS PASSWORD_MIN
        @constraint("must contain uppercase") AS PASSWORD_COMPLEXITY
```

### 12.4 External Model Integration (`MODEL`)

Some architectural properties (like distributed saga compensation or eventual consistency) cannot be verified by analyzing a single service's code locally. GASD 1.2 introduces the `MODEL` block to integrate with external formal provers like TLA+ or Alloy.
